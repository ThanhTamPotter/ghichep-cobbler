...
Ở đây khi thực hiện lab, có một vấn đề xảy ra với boot Ubuntu server 16.04 là lỗi không tìm được file kickstart. Sau khi tìm hiểu, tôi thấy bỏ đoạn cấu hình `auto-install/enable=true`  và thay thế đoạn file kickstart từ `url=` thành `ks=` trong menu boot của Ubuntu như sau là client boot file kickstart và cài đặt OS bình thường:  
