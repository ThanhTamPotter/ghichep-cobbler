...
cat <<HERE > /var/lib/cobbler/kickstarts/OPS_CTL.cfg
...
cobbler profile add --name=OPS-Controller --distro=US160403-x86_64 --kickstart=/var/lib/cobbler/kickstarts/OPS_CTL.cfg
pxedefault=/var/lib/tftpboot/pxelinux.cfg/default
cp $pxedefault $pxedefault.bak
sed -i 's/auto-install\/enable=true//g' $pxedefault
sed -i 's/url/ks/g' $pxedefault